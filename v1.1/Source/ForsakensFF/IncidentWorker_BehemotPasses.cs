...
{
    public class IncidentWorker_BehemotPasses : IncidentWorker
    {
        protected override bool CanFireNowSub(IncidentParms parms)
        {
            Map map = (Map)parms.target;
            IntVec3 intVec;
            return !map.gameConditionManager.ConditionIsActive(GameConditionDefOf.ToxicFallout) && map.mapTemperature.SeasonAndOutdoorTemperatureAcceptableFor(ForsakensFF_DefOf.FO_RBehemoth) && this.TryFindEntryCell(map, out intVec);
        }
